...
This project's purpose is to use various techniques to study the similarities and the differences in writing among three horror story authors, Edgar Allan Poe, HP Lovecraft, and Mary Shelley. ____________
...
packages.used <- c("ggplot2", "dplyr", "tidytext", "wordcloud", "stringr", "ggridges", "SnowballC", "tidyr")
...
This section is to study the most frequently used words 
...
```{r}
words_EAP_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "EAP",], word))$word
freqs_EAP_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "EAP",], word))$n

words_HPL_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "HPL",], word))$word
freqs_HPL_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "HPL",], word))$n

words_MWS_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "MWS",], word))$word
freqs_MWS_stem <- count(group_by(spooky_wrd_stem[spooky_wrd_stem$author == "MWS",], word))$n


wordcloud(words_EAP_stem, freqs_EAP_stem, max.words = 30, color = c("yellow", "orange", "red"))
wordcloud(words_HPL_stem, freqs_HPL_stem, max.words = 30, color = c("yellow", "orange", "red"))
wordcloud(words_MWS_stem, freqs_MWS_stem, max.words = 30, color = c("yellow", "orange", "red"))
```
...
3. 'Dai' is used frequently by all three authors.

## 6.4 Accented Words Dataset

### 6.4.i Frequency/Proportion of Words
...
# Counts number of times each word was used.
all_words_accented    <- rename(count(group_by(spooky_wrd_accented, word)), all = n)

author_words_accented <- left_join(author_words_accented, all_words_accented, by = "word")
author_words_accented <- arrange(author_words_accented, desc(all))
author_words_accented <- ungroup(head(author_words_accented, 60))
...
We see that only EAP and HPL uses these accented words. However, there isn't many overlap of the specific words used between the two authors. As a result, it may be more useful to just simply see the proportion of accented words used by each author.
...
As we can see, EAP has more than double the proportion of accented words compared to that of HPL. MWS doesn't use accented words at all.

##6.4.ii Word Clouds

Just for fun, I created wordclouds for these accented words for these two authors.

```{r}
words_EAP_accented <- count(group_by(spooky_wrd_accented[spooky_wrd_accented$author == "EAP",], word))$word
freqs_EAP_accented <- count(group_by(spooky_wrd_accented[spooky_wrd_accented$author == "EAP",], word))$n

words_HPL_accented <- count(group_by(spooky_wrd_accented[spooky_wrd_accented$author == "HPL",], word))$word
freqs_HPL_accented <- count(group_by(spooky_wrd_accented[spooky_wrd_accented$author == "HPL",], word))$n


wordcloud(words_EAP_accented, freqs_EAP_accented, max.words = 30, color = c("yellow", "orange", "red"))
wordcloud(words_HPL_accented, freqs_HPL_accented, max.words = 30, color = c("yellow", "orange", "red"))
```

Observations:

There isn't much overlap of common accented words between the two authors.

## 6.5 Bigram Dataset

### 6.5.i Frequency of Bigrams
...
MWS' 'Lord Raymond' is the most commonly used bigram among all three authors. 'Ha ha' is used most commonly by EAP. 'Heh heh' is used most frequently by HPL.

Among the top 30 bigrams, EAP holds the majority of them, whereas MWS holds the least.

### 6.5.ii Tf_idf 

Just for fault-proof, a Tf_idf analysis is also done to look at the rarest bigrams for each author.
...
bigram_tf_idf <- group_by(bigram_tf_idf, author)
bigram_tf_idf <- ungroup(head(bigram_tf_idf, 30))
